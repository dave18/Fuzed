...
int Tmine::loadgfx(SDL_Surface** s, char* n)
{
...
void Tmine::drawSprite(SDL_Surface* imageSurface, SDL_Surface* screenSurface, int srcX, int srcY, int dstX, int dstY, int width, int height,int id)
{
	SDL_Rect srcRect;
	srcRect.x = srcX;
	srcRect.y = srcY;
...
void Tmine::SetScale(float xscale,float yscale,int id)
{
...
int Tmine::spawn(int mx,int my,int sframe,int eframe,int acount,int bel) //Create new mine);
{
    int b;
    b=create();
    //printf("Spawn mine %d\n",b);
    ...
    item_store[b].time=40;						//Set time (fuse length)
    item_store[b].belongsto=bel;				//Record which player placed mine
    ...
    item_store[b].image=mine_floor;			//Image gfx
    item_store[b].zx=1;
    item_store[b].zy=1;
...
void Tmine::Update2(class_struct *is)
{
		...
		{
			is->time=is->time-1;
			if (is->time==0) channel=Mix_PlayChannel(-1,mine_arm_snd,0);
		}
		...
		{
			CollideImage (is->image,is->x,is->y,is->currframe*16,0,16,16,0,COL_MINE,is->id);
		}
}
