...
void Tslinky::init()
{
    loadgfx(&slinky_all,(char*)"graphics/slinky.png");
    loadgfx(&slinky_fall,(char*)"graphics/slinky_fall.png");
    loadgfx(&wheelie_die_left,(char*)"graphics/wheelie_die_left.png");
    loadgfx(&wheelie_die_right,(char*)"graphics/wheelie_die_right.png");
}

int Tslinky::loadgfx(SDL_Surface** s, char* n)
{
...
void Tslinky::drawSprite(SDL_Surface* imageSurface, SDL_Surface* screenSurface, int srcX, int srcY, int dstX, int dstY, int width, int height,int id)
{
	SDL_Rect srcRect;
	srcRect.x = srcX;
	srcRect.y = srcY;
...
void Tslinky::SetScale(float xscale,float yscale,int id)
{
...
int Tslinky::spawn(int px,int py,int dir)
{
    int e;
    e=create();
    if (e>=0)  //valid slot
    {
    item_store[e].x=SnaptoGrid16(px);
    item_store[e].y=SnaptoGrid16(py)+2;
    item_store[e].dx=dir;					//X direction
    item_store[e].startframe=0;		//Start anim frame
    item_store[e].endframe=31;			//End anim frame
    item_store[e].framecount=1;		//Counter for animation delay
    item_store[e].value=200;
    if (dir==1) item_store[e].currframe=item_store[e].startframe; else item_store[e].currframe=item_store[e].endframe;		//Current anim frame
...
void Tslinky::Update2(class_struct *is)
{
		...
		if (is->explode==0)
        {
			...
			if ((is->isjumping==0) && (is->currframe==15))
			{
				int xdrop=((is->x+16) >> 4);  //Check for playform beneath feet
				if (is->dx==-1) xdrop+=is->dx;
                ...
                {
					if (map[3] [xdrop] [ydrop]!=8)				//If not then fall
					{
						int xgrid,ygrid;
                        //xgrid=((is->x) >> 4) + 1;
						xgrid=xdrop;
						if ((xgrid>=0) && (xgrid<40))
                        {
                            ...
                            if (topofcheck>29) topofcheck=29;
                            if (isplatform==0)
                            {
                                for (int z=topofcheck;z>=1;z--)
                                {
                                    if (map[3] [xgrid] [z]==8) isplatform=z;
                                }
                            }
			...
			//'If isjumping=0 Then
				...
				if (is->framecount==0)
				{
					...
					is->currframe=is->currframe+is->dx;
					if (is->currframe>is->endframe)
					{
						if (is->isjumping==0)
						{
							is->currframe=is->startframe;
							is->x=is->x+16;
							if (is->x>640) is->x=-32;
							int xdrop=((is->x+16) >> 4);  //Check for platform beneath feet
							int ydrop=((is->y+16) >> 4)+1;
							if (xdrop>0 && xdrop<40)
							{
								if (map[3] [xdrop] [ydrop]==16)
								{
									is->dx=-is->dx;
									is->currframe=is->endframe;
									is->x-=16;
								}
							}
						}
					...
					if (is->currframe<is->startframe)
					{
						if (is->isjumping==0)
						{
							...
							is->x=is->x-16;
							if (is->x<-32) is->x=640;
					 	 	int xdrop=((is->x+16) >> 4)-1;  //Check for platform beneath feet
							int ydrop=((is->y+16) >> 4)+1;
							if ((xdrop>0) && (xdrop<40))
							{
								if (map[3] [xdrop] [ydrop]==2)
								{
									is->dx=-is->dx;
									is->currframe=is->startframe;
									is->x+=16;
			...
			if (is->isjumping==1)
			{
				...
				{
					is->oldy+=is->jumpdir;
					if (is->oldy>=480)  is->oldy=0;
					is->jumpsegs-=1;
				}
				is->image=slinky_fall;
				if (is->dx<0) is->fallframe=0; else is->fallframe=1;
				//'If y<=jumplevel-8 Then isjumping=0
				//'If (jumpdir<0 And y<=jumplevel) Or (jumpdir>0 And y>=jumplevel) Then
				if (((is->y >> 3) << 3) == is->jumplevel)
				{
					is->isjumping=2;
					is->y=is->jumplevel+2;
				}
...
void Tslinky::Draw()
{
    ...
    for (z=0;z<item_count;z++)
    {
		...
		if (item_store[z].isjumping>0)
		{
			/*if (item_store[z].isjumping==1)
			{
				drawSprite(item_store[z].image,screen,item_store[z].currframe*32,0,item_store[z].x,item_store[z].oldy,32,8,z);
				if (item_store[z].explode==0) CollideImage (item_store[z].image,item_store[z].x,item_store[z].oldy,item_store[z].currframe*32,0,32,8,0,COL_ENEMY,item_store[z].id);
			}*/
			int tempy=0;
			for (int i=0;i<item_store[z].jumpsegs;i++)
			{

				tempy=item_store[z].oldy+32+i*8;
				if (tempy>=480) tempy-=480;
				drawSprite(item_store[z].image,screen,item_store[z].fallframe*32,0,item_store[z].x,tempy,32,8,z);
				if (item_store[z].explode==0) CollideImage (item_store[z].image,item_store[z].x,tempy,item_store[z].fallframe*32,0,32,8,0,COL_ENEMY,item_store[z].id);
			}
...
int Tslinky::SnaptoGrid16(int v)
{
	return ((v >> 4) << 4);
}

void Tslinky::ClearList(void)
{
    if (item_count>0)
    {
        for (int z=0;z<item_count;z++)
