...
Tpickedup16::Tpickedup16()
{
    item_count=0;
}

void Tpickedup16::init()
{
    loadgfx(&picked_up_16x16,(char*)"graphics/picked_up_16x16.png");
}

int Tpickedup16::loadgfx(SDL_Surface** s, char* n)
{
...
void Tpickedup16::drawSprite(SDL_Surface* imageSurface, SDL_Surface* screenSurface, int srcX, int srcY, int dstX, int dstY, int width, int height,int id)
{
	SDL_Rect srcRect;
	srcRect.x = srcX;
	srcRect.y = srcY;
...
void Tpickedup16::SetScale(float xscale,float yscale,int id)
{
...
int Tpickedup16::spawn(int x, int y) //Create new pickedup16);
{
    int b;
    b=create();
    //printf("Spawn pickedup16 %d\n",b);
    if (b>=0)  //valid slot
    {
...
void Tpickedup16::Update2(class_struct *is)
{
		is->framecount+=1;
		if (is->framecount>2)
		{
...
void Tpickedup16::Destroy(int id)
{
...
void Tpickedup16::ClearList(void)
{
    if (item_count>0)
    {
        for (int z=0;z<item_count;z++)
