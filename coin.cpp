...
void Tcoin::init()
{
    loadgfx(&spinning_coin_gold,(char*)"graphics/spinning_coin_gold.png");
    loadgfx(&spinning_coin_silver,(char*)"graphics/spinning_coin_silver.png");
    loadgfx(&coin_collide,(char*)"graphics/coin_collide.png");

}

int Tcoin::loadgfx(SDL_Surface** s, char* n)
{
...
void Tcoin::drawSprite(SDL_Surface* imageSurface, SDL_Surface* screenSurface, int srcX, int srcY, int dstX, int dstY, int width, int height,int id)
{
	SDL_Rect srcRect;
	srcRect.x = srcX;
	srcRect.y = srcY;
...
void Tcoin::SetScale(float xscale,float yscale,int id)
{
...
int Tcoin::spawn(int px,int py, int acount, int t, int ft) //Create new coin);
{
    int g;
    g=create();
    if (g>=0)  //valid slot
    {
		...
		{
			case 1:
				item_store[g].value=5;
				item_store[g].image=spinning_coin_gold;
				break;
			default:
				item_store[g].value=1;
				item_store[g].image=spinning_coin_silver;
				break;
		}
...
void Tcoin::Update2(class_struct *is)
{
        if (is->framecount==0)
        {
		...
		if (is->fall==1)
		{
			if (is->falltime==0)
			{
				if (CollideImage(coin_collide,is->x,is->y+2,0,0,16,16,COL_FOREG)) is->fall=0;
			}
			else
			{
				is->falltime=is->falltime-1;
				if ((is->falltime==0) && (CollideImage(coin_collide,is->x,is->y,0,0,16,16,COL_FOREG))) is->falltime=1;
			}
		}
		...
		{
			CollideImage(is->image,is->x,is->y,is->currframe*16,0,16,16,0,COL_COIN,is->id);
			//printf("coin id=%d\n",is->id);
		}
